...
const App: React.FC = () => {
  const [isAuthenticated, setIsAuthenticated] = usePersistentState<boolean>(
  ...
  const [initialAction, setInitialAction] = useState<NavigationAction | null>(
    null,
  );
  ...
  const [teamProjectPayments, setTeamProjectPayments] = useState<
    TeamProjectPayment[]
  >([]);
  const [teamPaymentsLoaded, setTeamPaymentsLoaded] = useState(false);
  const [teamPaymentRecords, setTeamPaymentRecords] = useState<
    TeamPaymentRecord[]
  >([]);
  ...
  const [rewardLedgerEntries, setRewardLedgerEntries] = useState<
    RewardLedgerEntry[]
  >([]);
  ...
  useEffect(() => {
    try {
      if (!Array.isArray(rewardLedgerEntries) || teamMembers.length === 0)
        return;
      const totals: Record<string, number> = {};
      for (const entry of rewardLedgerEntries) {
        if (!entry?.teamMemberId) continue;
        totals[entry.teamMemberId] =
          (totals[entry.teamMemberId] || 0) + Number(entry.amount || 0);
      }
      setTeamMembers((prev) => {
        let changed = false;
        const next = prev.map((m) => {
          const computed = Object.prototype.hasOwnProperty.call(totals, m.id)
            ? totals[m.id]
            : 0;
      ...
      console.warn(
        "[Rewards] Failed to recompute reward balances from ledger.",
        e,
      );
    }
  ...
  useEffect(() => {
    const channel = supabase
      .channel("realtime-reward-ledger-entries")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "reward_ledger_entries" },
        (payload) => {
          console.log("RewardLedgerEntry change received!", payload);
          if (payload.eventType === "INSERT") {
            setRewardLedgerEntries((current) => [
              payload.new as RewardLedgerEntry,
              ...current,
            ]);
          }
          if (payload.eventType === "UPDATE") {
            setRewardLedgerEntries((current) =>
              current.map((e) =>
                e.id === (payload.new as any).id
                  ? ({ ...e, ...payload.new } as RewardLedgerEntry)
                  : e,
              ),
            );
          }
          if (payload.eventType === "DELETE") {
            setRewardLedgerEntries((current) =>
              current.filter((e) => e.id !== (payload.old as any).id),
            );
          }
        },
      )
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const remoteUsers = await listUsersFromDb();
        if (!isMounted) return;
  ...
  useEffect(() => {
    ...
    // 2. Realtime subscription
    const channel = supabase
      .channel("realtime-client-feedback")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "client_feedback" },
        (payload) => {
          console.log("Client feedback change received!", payload);
          if (payload.eventType === "INSERT") {
            setClientFeedback((current) =>
              [payload.new as ClientFeedback, ...current].sort(
                (a, b) =>
          ...
          if (payload.eventType === "UPDATE") {
            setClientFeedback((current) =>
              current.map((f) =>
                f.id === payload.new.id
                  ? ({ ...f, ...payload.new } as ClientFeedback)
                  : f,
              ),
            );
          }
          if (payload.eventType === "DELETE") {
            setClientFeedback((current) =>
              current.filter((f) => f.id !== (payload.old as any).id),
            );
          }
        },
      )
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn(
          "[Supabase] Failed to fetch cards, falling back to local data.",
          e,
        );
      }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn(
          "[Supabase] Failed to fetch promo codes, falling back to local data.",
          e,
        );
      }
    })();
  ...
  useEffect(() => {
    ...
    if (newView) {
      ...
      // Handle 'Freelancer' mapping to 'team' route
              ...
              onRecordPayment={async (
                ...
                try {
                  ...
                  const tx = await createTransaction({
                    date: today,
                    description: `Pembayaran Acara ${proj.projectName}`,
                    amount,
