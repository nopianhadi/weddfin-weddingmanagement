...
const Packages: React.FC<PackagesProps> = ({ packages, setPackages, addOns, setAddOns, projects, profile }) => {
    ...
    const handlePackageDelete = async (pkgId: string) => {
        ...
        if (isPackageInUse) {
            alert("Paket ini tidak dapat dihapus karena sedang digunakan oleh satu atau lebih acara. Hapus atau ubah acara tersebut terlebih dahulu.");
            return;
        }
    ...
    const handleAddOnDelete = async (addOnId: string) => {
        ...
        if (isAddOnInUse) {
            alert("Add-on ini tidak dapat dihapus karena sedang digunakan oleh satu atau lebih acara. Hapus atau ubah acara tersebut terlebih dahulu.");
            return;
        }
    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-fade-in pb-8">
            <PageHeader title="Paket & Layanan" subtitle="Kelola portofolio paket, opsi durasi, dan item tambahan (add-ons).">
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button onClick={() => setPackageEditMode('new')} className="button-primary !py-2 !px-4 text-xs md:text-sm inline-flex items-center gap-2 whitespace-nowrap">
                        <PlusIcon className="w-4 h-4 md:w-5 md:h-5" /> Tambah Paket
                    </button>
                </div>
            </PageHeader>
                                    ...
                                    <div key={pkg.id} className="glass-card card-hover-lift rounded-3xl flex flex-col overflow-hidden border border-brand-border/50 group">
                                        ...
                                        <div className="p-4 md:p-5 flex-grow flex flex-col bg-brand-surface/40">
                                            ...
                                            <div className="text-xs space-y-3 flex-grow bg-white/30 p-3 rounded-xl border border-white/40 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)]">
                                                ...
                                                {pkg.digitalItems.length > 0 && (
                                                    <div className="pb-2 border-b border-brand-border/40 last:border-0 last:pb-0">
                                                        <h5 className="font-semibold text-brand-text-secondary text-[9px] uppercase tracking-wider mb-1.5">Deskripsi Paket</h5>
                                            ...
                                            <div className="flex gap-2 mt-4 pt-4 border-t border-brand-border/50">
                                                <button onClick={() => handlePackageEdit(pkg)} className="button-secondary flex-1 text-xs py-2 shadow-sm">Edit Paket</button>
                                                <button onClick={() => handlePackageDelete(pkg.id)} className="button-secondary !p-2 md:!p-2.5 text-brand-text-secondary hover:text-red-600 hover:border-red-300 hover:bg-red-50 flex-shrink-0" title="Hapus Paket"><Trash2Icon className="w-4 h-4" /></button>
                                            </div>
                                        </div>
                                    </div>
                                ))}
            ...
            <Modal isOpen={packageEditMode !== null} onClose={handlePackageCancelEdit} title={packageEditMode === 'new' ? 'Tambah Paket Baru' : 'Edit Paket'} size="3xl">
                <form onSubmit={handlePackageSubmit} className="space-y-5 md:space-y-6 max-h-[70vh] overflow-y-auto pr-2 pb-4 form-compact form-compact--ios-scale">
                    ...
                    <section className="bg-white/40 md:bg-transparent rounded-2xl md:rounded-none p-4 md:p-0 border md:border-0 border-brand-border/40">
                        <h4 className="text-sm md:text-base font-semibold text-gradient border-b border-brand-border/40 pb-2 mb-4">Informasi Dasar Paket</h4>
                        <p className="text-xs text-brand-text-secondary mb-4">Masukkan nama dan harga paket layanan Anda. Nama harus jelas dan menarik untuk pengantin.</p>
                            ...
                            <div key={index} className="mt-2 border border-brand-border/40 bg-white/40 rounded-xl overflow-hidden shadow-sm">
                                <div className="grid grid-cols-1 md:grid-cols-5 gap-2 items-center p-3 border-b border-brand-border/30 bg-brand-surface/40">
                                    ...
                                    <div className="flex items-center justify-end gap-1 md:gap-2">
                                        ...
                                        <button type="button" onClick={() => setExpandedDurationIndex(expandedDurationIndex === index ? null : index)} className="p-1.5 rounded hover:bg-brand-input text-brand-accent" title="Detail paket">
                                ...
                                {expandedDurationIndex === index && (
                                    <div className="p-3 border-t border-brand-border bg-brand-surface space-y-3">
                                        ...
                                        <div>
                                            <p className="text-xs text-brand-text-secondary mb-1">Deskripsi Paket</p>
                    ...
                    <section className="bg-white/40 md:bg-transparent rounded-2xl md:rounded-none p-4 md:p-0 border md:border-0 border-brand-border/40">
                        ...
                        <p className="text-xs text-brand-text-secondary mb-4">Pilih kategori paket dan tentukan wilayah layanan. Wilayah membantu pengantin menemukan paket yang sesuai dengan lokasi mereka.</p>
                        <div className="input-group">
                    ...
                    <section className="bg-white/40 md:bg-transparent rounded-2xl md:rounded-none p-4 md:p-0 border md:border-0 border-brand-border/40">
                        ...
                        <p className="text-xs text-brand-text-secondary mb-4">Informasi tentang jumlah tim yang akan ditugaskan untuk paket ini.</p>
                    ...
                    {/* Section 6: Deskripsi Paket */}
                    <section className="bg-white/40 md:bg-transparent rounded-2xl md:rounded-none p-4 md:p-0 border md:border-0 border-brand-border/40">
                        <h4 className="text-sm md:text-base font-semibold text-gradient border-b border-brand-border/40 pb-2 mb-4">Deskripsi Paket</h4>
                    ...
                    <div className="flex flex-col md:flex-row justify-end items-stretch md:items-center gap-3 pt-4 mt-6 border-t border-brand-border/40 sticky -bottom-4 md:bottom-0 bg-white/90 md:bg-white/80 backdrop-blur-xl p-4 -mx-4 md:p-3 md:mx-0 z-10 rounded-b-2xl md:rounded-b-none shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
                        ...
                        <button type="submit" className="button-primary w-full md:w-auto order-1 md:order-2 shadow-md min-w-[120px] py-2 md:py-2.5">{packageEditMode === 'new' ? 'Simpan Paket' : 'Update Paket'}</button>
                    </div>
                </form>
            </Modal>
            ...
            <Modal isOpen={isInfoModalOpen} onClose={() => setIsInfoModalOpen(false)} title="Panduan Halaman Paket">
                <div className="space-y-4 text-sm text-brand-text-primary">
                    ...
                    <ul className="list-disc list-inside space-y-2">
                        ...
                        <li><strong>Bagikan Halaman Paket:</strong> Gunakan tombol di kanan atas untuk mendapatkan tautan ke halaman publik yang menampilkan semua paket Anda, siap untuk dibagikan kepada calon pengantin.</li>
