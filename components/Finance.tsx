...
const PRODUCTION_COST_CATEGORIES = ["Gaji Freelancer", "Transportasi", "Konsumsi", "Sewa Tempat", "Sewa Alat", "Produksi Fisik"];
...
const Finance: React.FC<FinanceProps> = ({ transactions, setTransactions, pockets, setPockets, projects, profile, cards, setCards, teamMembers, rewardLedgerEntries, setRewardLedgerEntries }) => {
    const [activeTab, setActiveTab] = useState<'transactions' | 'pockets' | 'cards' | 'cashflow' | 'laporan' | 'laporanKartu' | 'labaAcara'>('transactions');
    ...
    const getTransactionSubDescription = (transaction: Transaction): string => {
        ...
        if (isInternal) {
            return projectText ? `Acara: ${projectText}` : '';
        }
    ...
    const handleDownloadProfitReportCSV = () => {
        ...
        downloadCSV(headers, data, `Laporan-Laba-Acara-${period}-${new Date().toISOString().split('T')[0]}.csv`);
    };
                                ...
                                return (
                                    <div key={p.id} className="bg-brand-surface p-6 rounded-2xl shadow-lg flex flex-col cursor-pointer border border-brand-border" onClick={() => setHistoryModalState({ type: isRewardPool ? 'reward_pool' : 'pocket', item: p })}>
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <p className="font-bold text-lg text-gradient flex items-center gap-2">{p.name}</p>
                                                <p className="text-sm text-brand-text-secondary">{p.description}</p>
                                            </div>
                                        ...
                                        <div className="flex-grow mt-4"><p className="text-2xl font-bold text-brand-text-light">{formatCurrency(amount)}</p></div>
                    ...
                    <div className="space-y-6 printable-area widget-animate">
                        ...
                        <div className="md:hidden space-y-3">
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <div className="rounded-2xl bg-white/5 border border-brand-border p-3"><p className="text-[11px] text-brand-text-secondary">Avg/Acara</p><p className="font-semibold">{formatCurrency(profitReportMetrics.avgProfit)}</p></div>
                            </div>
                            <div className="rounded-2xl bg-white/5 border border-brand-border p-4">
                        ...
                        <div className="hidden md:grid grid-cols-2 lg:grid-cols-4 gap-4 non-printable">
                            ...
                            <StatCard icon={<TrendingUpIcon className="w-6 h-6" />} title="Jumlah Acara Profit" value={`${profitReportMetrics.profitableProjectsCount} dari ${projectProfitabilityData.length}`} colorVariant="orange" />
                            <StatCard icon={<TargetIcon className="w-6 h-6" />} title="Rata-rata Laba/Acara" value={formatCurrency(profitReportMetrics.avgProfit)} colorVariant="purple" />
                        </div>
                        <div className="printable-report hidden md:block">
                            ...
                            <div className="bg-brand-surface p-6 rounded-2xl shadow-lg mt-6 border border-brand-border print:shadow-none print:border-none print:p-0 print:mt-0">
                                <div className="print:hidden">
                                    ...
                                    <p className="text-sm text-brand-text-primary mb-4">Menampilkan profitabilitas untuk acara yang dieksekusi pada <strong>{monthOptions.find(m => m.value === profitReportFilters.month)?.name} {profitReportFilters.year}</strong>.</p>
                                </div>
                                    ...
                                    <table className="w-full text-sm">
                                        <thead className="text-xs uppercase print-bg-slate bg-brand-input">
                                            <tr className="print-text-black">
                                                ...
                                                <th className="p-3 text-right">Harga Paket</th>
                                        ...
                                        <tbody className="divide-y divide-brand-border">
                                            ...
                                            {projectProfitabilityData.length === 0 && (
                                                <tr><td colSpan={5} className="text-center p-8 text-brand-text-secondary">Tidak ada data acara untuk periode ini.</td></tr>
                                            )}
                                        </tbody>
                                    </table>
                                </div>
                ...
                <div className="bg-brand-surface p-6 rounded-2xl shadow-lg border border-brand-border">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="p-4 bg-brand-bg rounded-xl border border-brand-border/50">
                            <p className="text-xs text-brand-text-secondary uppercase font-bold tracking-wider mb-1">Paket Utama</p>
        ...
        <div className="space-y-6">
            ...
            <div className="hidden md:block border-b border-brand-border non-printable widget-animate" style={{ animationDelay: '500ms' }}>
                <nav className="-mb-px flex space-x-6 overflow-x-auto">
                    ...
                    <button onClick={() => setActiveTab('labaAcara')} className={`shrink-0 inline-flex items-center gap-2 py-3 px-1 border-b-2 font-medium text-sm transition-colors ${activeTab === 'labaAcara' ? 'border-brand-accent text-brand-accent' : 'border-transparent text-brand-text-secondary hover:text-brand-text-light'}`}><DollarSignIcon className="w-5 h-5" />Laba Acara</button>
                </nav>
            </div>
